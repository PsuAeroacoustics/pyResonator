...
def route(sample,resonator):
    ...
    while len(open_set) > 0:
        ...
        if current_node.L == resonator.length:
            ...
            bounds = ((neighbors[:,0] >= sample.size[3]) | (neighbors[:,0] <= sample.size[2])) |((neighbors[:,1] >= sample.size[1]) | (neighbors[:,1] <= sample.size[0]))
            if np.any(bounds):
            ...
            for i, neighbor in enumerate(neighbors):
                ...
                if n in closed_set or n in sample.occupied_nodes:
                    neighbors = np.delete(neighbors,np.sum(neighbors==neighbor,axis = -1)==2,axis = 0)
            ...
            # start_time = time.time()
            # for i, neighbor in enumerate(neighbors):
            #     node(position = tuple(neighbor)) in closed_set_2
...
res_start = [(10,3),(10,5),(10,4)]
res_L = [25,25,6]

liner = sample(size = [0,10,0,10],N_res = 3)
...
for k,v in liner.resonators.items():
    if v.success:
        ...
        ax.scatter(parent_set[0,1],parent_set[0,0])
ax.set_xlim(0,10)
ax.set_ylim(0,10)
plt.grid()